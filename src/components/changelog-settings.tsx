  ...
  async function handleToggle() {
    ...
    try {
      const res = await fetch(`/api/projects/${projectId}`, {
        ...
        body: JSON.stringify({ publicChangelog: newValue }),
      });
      if (res.ok) {
  ...
  return (
    <div className="glow-card rounded-xl border border-zinc-800 bg-[#18181b] p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-zinc-100">Publish changelog</p>
          <p className="text-sm text-zinc-400">
            Auto-generate a changelog entry every time a feedback PR is merged.
          </p>
        </div>
        <button
          type="button"
